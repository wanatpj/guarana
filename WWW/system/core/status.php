...
//	print $mhost;
    ...
    <input name="whereAmI" value="status" type="hidden">
    ...
    $zapytanie="SHOW TABLE STATUS LIKE 'solutions'";
    $wynik=mysqli_query($mysql,$zapytanie);
    if(!$wynik) {
	print "Błąd.";
	exit;
    ...
    $id=$wiersz[4];
    //echo "<br><br><center>Ilość zgłoszeń: ".$id;
    ...
    $zapytanie="FROM solutions ";
    if(!isEmpty($_GET['zadanie'])&&$_GET['zadanie']!="all") {
	$w=1;
	$zapytanie=$zapytanie."WHERE `task ID`=$_GET[zadanie] ";
    } 
    if(!isEmpty($_GET['res'])&&$_GET['res']!="all") {
	if($w==1) $zapytanie=$zapytanie."AND result=100 ";
	else $zapytanie=$zapytanie."WHERE result=100 ";
    }    
    ...
    print '<table><tr>
    <td align=left><a href=index.php?whereAmI=status&od='.($od - 10).'';
    ...
    echo'</td>
    <td align=right><a href=index.php?whereAmI=status&od='.($od + 10).'';
    if(!isEmpty($_GET['zadanie'])) print '&zadanie='.$_GET['zadanie'].'';
    if(!isEmpty($_GET['res'])) print '&res='.$_GET['res'].'';
    print '>dalej</a></td>
    </tr></table>';
