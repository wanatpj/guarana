...
//						actNaw = nawiasy[j][i];
						...
						if(wyniki[j][i] > wyniki[j][p-j-1]+wyniki[p][k-p]+waga[j][i] || (wyniki[j][i] == wyniki[j][p-j-1]+wyniki[p][k-p]+waga[j][i] && nawiasy[j][i]>tempNaw)  )
						{
							wyniki[j][i]=wyniki[j][p-j-1]+wyniki[p][k-p]+waga[j][i];
							nawiasy[j][i] = tempNaw;
