...
void fajnd()
{
   ...
   while(!(x==kx&&y==ky))
   {
      ...
      Qy.push(y);
      ile++;
      if(x>1&&(tab[y][x]=='l'||tab[y][x]=='.')&&toend[y][x-1]&&!visited[y][x-1])
      x--;
      else if(x<n&&(tab[y][x]=='p'||tab[y][x]=='.')&&toend[y][x+1]&&!visited[y][x+1])
      x++;
      else if(y>0&&(tab[y][x]=='d'||tab[y][x]=='.')&&toend[y-1][x]&&!visited[y-1][x])
      y--;
      else if(y<n&&(tab[y][x]=='g'||tab[y][x]=='.')&&toend[y+1][x]&&!visited[y+1][x])
      y++;
   }
