...
struct cZone {
	...
	void knapsack() {
		...
		t[0]=0;
		for(int i=0; i<items.size(); i++) for(int j=upBound; j>0; j--) if(j>=items[i].val) t[j]=min(t[j-items[i].val]+items[i].time,t[j]);
	...
	for(int i=0; i<vEvents.size(); i++) {
		...
		if(vEvents[i].modeSeg) {
			if(vEvents[i].modeIn) {
				...
				sTree.erase(sTree.lower_bound(vEvents[i].segment));
			}
		} else {
			cSegment ins;
