...
inline bool SegmentCrossPoint( point p1, point p2, point l1, point l2, vector<point> &r ){
    ...
    if ( ! w1 && ! w2 ){
        ...
        if ( p2 == l1 ) r.push_back( point(p2.x, p2.y) );
            ...
            r.push_back( OrdXY(&p1, &l1) ? l1 : p1 );
            r.push_back( OrdXY(&p2, &l2) ? p2 : l2 );
        }
    }
    else {
...
double PolygonArea(  vector<point> &p ){
    double area = 0;
    int s = p.size();
    for (int x=0; x<s; ++x) area += ( p[x].x + p[ (x+1)%s ].x ) * ( p[x].y - p[ (x+1)%s ].y );
    return absol(area) / 2.0;
}


double il_wek_wsp(double x1, double y1, double x2, double y2){return x1*y2-x2*y1;}

double il_wek(point p1, point p2, point p3){
...
double pierwszy = PolygonArea( T1 );
double drugi = PolygonArea( T2 );
double przeciecie = PolygonArea( P );
