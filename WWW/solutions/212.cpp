...
main() {
       ...
       {
          inext = i == top-1 ? 0 : i + 1;
          wynik += ((stos[i].x)*(stos[inext].y) - (stos[i].y)*(stos[inext].x))/2;
       }
       ...
       REP(i,top)
       {
          ...
          if(stos[i].TR != stos[inext].TR) {
             ...
             #ifdef DBG
             printf("%.5lf %.5lf\n",stos[i].x,stos[i].y);
             printf("%.5lf %.5lf\n",stos[inext].x,stos[inext].y);
             printf("atacz: %.5lf %.5lf\n",atacz(stos[i],stos[inext]).x,atacz(stos[i],stos[inext]).y);
             #endif
          }
       }
