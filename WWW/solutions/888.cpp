...
struct cGraph {
	...
	cResult finalClique(int root) {
...
#endif
		cResult upResult, dnResult, bestResult;
		for(int i=problems.size()-1; i>=0; i--) {
			...
			upResult.dodaj(curUp);
			//if(curDn<dnResult) 
			dnResult=curDn;
			if(problems[i].downCount==0) dnResult=cResult();
			cResult curAll=dnResult;
			curAll.dodaj(upResult);
			sort(curAll.r.begin(), curAll.r.end());
			if(curAll<bestResult) bestResult=curAll;
		}
