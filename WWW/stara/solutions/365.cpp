...
void licz() {
     ...
     FOR(i,1,n)
     {
        //printf("%d %d\n",i,n - pustych - naniego[i] - (kogo[i]==i) - tylkonie[i] - pusty[i]);
        mi = min(mi, n - pustych - naniego[i] - (kogo[i]==i) - tylkonie[i] - pusty[i]);
     }
     FOR(i,1,n)
        if(n - pustych - naniego[i] - (kogo[i]==i) - tylkonie[i] - pusty[i] == mi)
           printf("%d ",i);
     printf("\n");
}
