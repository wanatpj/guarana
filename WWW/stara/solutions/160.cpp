//Czêœæ kodu zaczerpniêta z "Algorytmika praktyczna w konkursach Informatycznych" Piotra Stañczyka
#include <iostream>
#include <vector>
#define LD double
...
inline bool inside(const tro &a, COP b)
{
       int det1, det2;
       LD d1, d2;
       d1=det(a.p[0],a.p[1],b);
       d2=det(a.p[1],a.p[2],b);
       det1=sgn(d1);
       det2=sgn(d2);
       //cout << "det1: " << det(a.p[0],a.p[1],b) << " det2: " << det(a.p[1],a.p[2],b) << endl;
       //cout << "det1: " << det1 << " det2: " << det2 << endl;
       if(det1*det2>0||isZero(d1)||isZero(d2))
       {
            d2=det(a.p[2],a.p[0],b);
            det2=sgn(d2);
            if(det1*det2>0||isZero(d1)||isZero(d2)) return true;          
       }
       return false;       
}
...
LD otoczka()
{
       ...
       for(int i=2;i<V.size();i++) res+=abs(det(V[i-2],V[i-1],V[i]))/2.0;     
       return res;     
}
