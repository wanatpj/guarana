...
const int N = 5;
int tab[N][N],bity[N][N],chro[N][N],temp[N][N];
...
//  printf("szachuje pion na miejscu : %d,%d\n",szachx,szachy);
  for(int i=0;i<d;i++){
    if(mozna(piocx[i]+1,piocy[i]-1) && piocx[i]+1==szachx && piocy[i]-1==szachy) return false;
    if(mozna(piocx[i]+1,piocy[i]+1) && piocx[i]+1==szachx && piocy[i]+1==szachy) return false;    
  }
  for(int i=0;i<9;i++)
    if(mozna(czarx+krolx[i],czary+kroly[i]) && !bity[czarx+krolx[i]][czary+kroly[i]] && temp[czarx+krolx[i]][czary+kroly[i]]!=4) return false;
  return true;
//  printf(" --- po zestawie ---\n");
}
...
main(){
 ...
 //      printf("mam teraz piona : %d,%d\n",x,y);
     ...
     for(int i=0;i<c;i++) piocx[i]=piocy[i]=0;
   }
}
