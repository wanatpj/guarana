...
bool CzyAtakowany(int x, int y) //czy czarny krol moze uciec
{
    if(y>0)
    {
...
bool NieUcieknie(int x, int y)
{
    if(y>0)
    {
	if(x>0) if(!P[y-1][x-1] && !CzyAtakowany(x-1,y-1)) return false;
	if(!P[y-1][x] && !CzyAtakowany(x, y-1)) return false;
	if(x<3) if(!P[y-1][x+1] && !CzyAtakowany(x+1,y-1)) return false;
    }
    if(x>0) if(!P[y][x-1] && !CzyAtakowany(x-1,y)) return false;
    if(x<3) if(!P[y][x+1] && !CzyAtakowany(x+1, y)) return false;
    if(y<3)
    {
	if(x>0) if(!P[y+1][x-1] && !CzyAtakowany(x-1, y+1)) return false;
	if(!P[y+1][x] && !CzyAtakowany(x, y+1)) return false;
