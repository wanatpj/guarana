...
const int maxZones=1<<4+2;
const int maxItems=(1<<12)+1;
const int maxSegments=1<<16+2;
const int maxVal=(1<<2);
const int maxSum=(1<<8);
const int maxTime=(1<<12)+2;
...
struct cPoint {
	...
	struct cRange {
		lld a,b;
		cRange() {}
		cRange(lld aa, lld bb): a(aa), b(bb) {}
...
#ifdef DBG
		REP("seg ("<<a.x<<","<<a.y<<") ("<<b.x<<","<<b.y<<") insUp "<<insideUp<<" ZONE "<<zone);
#endif
	}
};
...
struct cZone {
	vector<cItem> items;
	int t[2*maxItems*maxVal];
	int v[2*maxItems*maxVal];	
	int upBound,realSum;
	void init() { items.clear(); }
	void knapsack(bool last) {
		upBound=realSum=0;
		for(int i=0; i<items.size(); i++) if(items[i].val>0) upBound+=items[i].val; else realSum+=items[i].val;
		realSum+=upBound;
		for(int i=0; i<=upBound; i++) t[i]=inf;
		t[0]=0;
		for(int i=0; i<items.size(); i++) for(int j=upBound; j>0; j--) if(j>=items[i].val&&items[i].val>0) 
			t[j]=min(t[j-items[i].val]+items[i].time,t[j]);
		int reach=realSum+(abs(realSum)*items.size()+99)/100;
		REP("reach "<<reach<<" realSum "<<realSum<<" upb "<<upBound);
		if(reach>0&&!last) {
		REP("upb1 "<<upBound);
			for(int i=upBound+1; i<=reach; i++) t[i]=inf;
			int tmr=0;
		REP("upb2 "<<upBound);
		...
		//for(int i=0; i<=upBound; i++) if(t[i]<inf) cout<<i<<":"<<t[i]<<" ";
		cout<<endl;
#endif
	}
	void dDescribe() {
...
void readData() {
	...
	for(int d=0; d<nZones; d++) {
		tZones[d].init();
		int p; scanf("%d", &p);
		for(int i=0; i<p; i++) scanf("%lld%lld", &buf[i].x, &buf[i].y);
		// should be clockwise!
		{ // check for rightmost element 
			lld r=0;
			for(int i=1; i<p; i++) if(buf[r]<buf[i]) r=i;
			if((buf[r]-buf[md(r-1,p)])*(buf[md(r+1,p)]-buf[r])>0) for(int i=0; i<p/2; i++) swap(buf[i],buf[p-i-1]);
		}
		for(int i=0; i<p; i++) {
			cSegment o;
			o.uid=uid++;
	...
	for(int i=0; i<nItems; i++) {
		cEvent item;
		scanf("%lld%lld%d%d", &item.p.x, &item.p.y, &item.item.time, &item.item.val);
		item.modeSeg=0;
...
void redefineProblem() {
	...
	for(int i=0; i<vEvents.size(); i++) {
		...
		if(vEvents[i].modeSeg) {
			...
			set<cSegment>::iterator it,it2;
			it=it2=sTree.find(ins);
			if(!sTree.size()||it==sTree.begin()||!(--it2)->insideUp) {
				tZones[nZones-1].items.push_back(vEvents[i].item);
				continue;
			}
			cSegment f=*(--it);
			ins.dDescribe();
...
void finallyDynamic() {
	Sum=0;
	for(int i=0; i<nZones; i++) tZones[i].knapsack(i==nZones-1);
	...
	for(int i=0; i<nZones; i++) {
		cout<<i<<": ";
		for(int j=0; j<=Sum; j++) if(tZones[i].v[j]!=inf) cout<<j<<":"<<tZones[i].v[j]<<" ";
		cout<<endl;
	}
#endif
}
...
void forumMod() {
	nZones=maxZones-1;
	for(int i=0; i<nZones; i++) tZones[i].init();
	int n,t; scanf("%d%d", &n, &timeLimit);
	while(n--) {
		int dv,dt,dz; scanf("%d%d%d", &dv, &dt, &dz);
		if(dz==-1) dz=nZones-1;
		cItem it;
		it.val=dv;
		it.time=dt;
		tZones[dz].items.push_back(it);
	}
