...
Ld polePrze() // pole przeciecia
{
    ...
    for(vector<pkt>::iterator iter = przeciecia.begin();iter!=przeciecia.end();iter++)
        if ( (iter->x == doCzego.x) && (iter->y==doCzego.y))
        {
            przeciecia.erase(iter);
            break;
        }
    sort(przeciecia.begin(), przeciecia.end(), Cmp); // przesortowac zawsze dobrze
    przeciecia.insert(przeciecia.begin(),doCzego);
...
bool wTrojkacie(pkt A, bool ktore)
{
    if(ktore)
    {
        Ld d1=det(tab[3], tab[4], A);
        Ld d2=det(tab[4], tab[5], A);
        Ld d3=det(tab[5], tab[3], A);
        if(d1<0 && d2<0 && d3<0) return true;
        if(d1>0 && d2>0 && d3>0) return true;
    }
    else
    {
        Ld d1=det(tab[0], tab[1], A);
        Ld d2=det(tab[1], tab[2], A);
        Ld d3=det(tab[2], tab[  0], A);
        if(d1<0 && d2<0 && d3<0) return true;
        if(d1>0 && d2>0 && d3>0) return true;
    }
    return false;
...
bool zawiera(bool ktore)
{
    ...
    for(int i=3; i<6; i++)
    {
        if(!wTrojkacie(tab[i],false)) return false;
    }
    else
    for(int i=0; i<3; i++)
    {
        if(!wTrojkacie(tab[i],true)) return false;
    }
    return true;
}
    ...
    for(int i=0; i<3; i++) // wrzucaj przeciecia
        for(int j=0; j<3; j++)
        {
            if(CzyPrzecinaja(bokiT1[i], bokiT2[j])) // jesli sie przecinaja
            {
                ...
                //Przeciecie(bokiT1[i], bokiT2[j]).Wypisz();
            }
        }
    for(int i=0; i<6; i++)
    {
        if(i<3)
        {
            if(wTrojkacie(tab[i],true))
            {   przeciecia.push_back(tab[i]);
                //tab[i].Wypisz();
            }
        }
        else if(wTrojkacie(tab[i], false))
        {
            przeciecia.push_back(tab[i]);
            //tab[i].Wypisz();
        }
