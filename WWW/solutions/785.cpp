...
//        printf("przed podzialem: p=%d k=%d\n",p,k);
 /*       for (int i=p; i<=k; i++) {
...
int rekNSP(int p, int k) {
        ...
        sort(wierzcholek+p,wierzcholek+k+1,pornr);
        int pp = p;
        int pw=p,kw=p,mw,aw,ilekr;
        mw = 0;
        ilekr = wierzcholek[p]->lista.size();
        for (int i=p+1; i<=k; i++)
                if (wierzcholek[i]->spojna != wierzcholek[i-1]->spojna) {
/*                        printf("spojna nr %d, powinna miec %d krawedzi: ",wierzcholek[i-1]->spojna,ilekr/2);
                        for (int j=pp; j<i; j++)
                        ...
                        if (aw > mw) { // jezeli ta spojna ma wiekszy podgraf pelny
                                ...
                                min2 = wierzcholek[pw]->nr; // sprawdzamy najmniejszy nr drugiego
                                for (int j=pp; j<i; j++)
