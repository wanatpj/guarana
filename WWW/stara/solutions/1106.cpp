...
int main()
{
	...
	while(T>0)
	{
		...
		FORD(i,m-1,0)
		{
			...
			if(z[i]!=0)
			{
				a=upper_bound(ALL(l[z[i]-1]),i,por)-l[z[i]-1].begin();
				poz=lower_bound(ALL(l[z[i]]),i)-l[z[i]].begin();
				if(poz>0 && s[l[z[i]][poz-1]]==s[i])
					a=max(a,upper_bound(ALL(l[z[i]-1]),l[z[i]][poz-1])-l[z[i]-1].begin());
				b=lower_bound(ALL(l[z[i]-1]),i)-l[z[i]-1].begin()-1;
				if(b>=a)
		...
		REP(i,il)
		{
			...
			REP(j,poz)
			{
				...
				{
					//printf("%d %d %d %lld %lld %lld\n", j, kol[j], t[kol[j]], k, find(q[kol[j]]), licz);
					if(wyw==t[kol[j]])
						continue;
					//printf("%d %d %d %lld %lld %lld\n", j, kol[j], t[kol[j]], k, find(q[kol[j]]), licz);
					if(ok)
