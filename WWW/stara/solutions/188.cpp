...
struct cPointD {
	...
	double operator*(const cPointD& rhs) const {
		return (x)*rhs.y-(y)*rhs.x;
	}
	bool operator<(const cPointD& rhs) const {
		bool wyn= (*this)*rhs>0;
		REPORT(x<<","<<y<<" < "<<rhs.x<<","<<rhs.y<<": "<<wyn);
		return wyn;
	}
	int cw() {
		if(x>=0&&y>=0) return 0;
...
double area(vector<cPointD> v) {
	...
	REPORT("area licznosci "<<v.size()<<" pole "<<a/2);
	if(a<0) return a/-2;
	return a/2;
}

void anglesort(vector<cPointD>& all) {
	REPORT("Ang "<<all.size());
...
int main()
{
	...
	cPointD cen(0,0);
	for(int i=0; i<all.size(); i++) { cen.x+=all[i].x/all.size(); cen.y+=all[i].y/all.size(); }
	REPORT("cen "<<cen.x<<","<<cen.y);
	for(int i=0; i<all.size(); i++) { all[i].x-=cen.x; all[i].y-=cen.y; }
	vector<cPointD> cw[4];
	for(int i=0; i<all.size(); i++)
		cw[all[i].cw()].push_back(all[i]);
	for(int i=0; i<4; i++) anglesort(cw[i]);
	all.clear();
	for(int i=0; i<4; i++) all.insert(all.end(),cw[i].begin(), cw[i].end());
	for(int i=0; i<all.size(); i++) REPORT("all "<<all[i].x<<" "<<all[i].y);
