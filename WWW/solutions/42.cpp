//sprawdzic czy czarne moga wykonac ruch !!!
#include <iostream>
#define MAX 4
using namespace std;
...
void atak()
{
     for(int i=0;i<MAX;i++) for(int j=0;j<MAX;j++) ak[i][j]=0;
     if(Ca.y>0)
     {
           ak[Ca.y-1][Ca.x]=1;
           if(Ca.x>0) ak[Ca.y-1][Ca.x-1]=1;
           if(Ca.x<3) ak[Ca.y-1][Ca.x+1]=1;          
     }
     if(Ca.y<3)
     {
           ak[Ca.y+1][Ca.x]=1;
     ...
     if(Ca.x>0) ak[Ca.y][Ca.x-1]=1;
     if(Ca.x<3) ak[Ca.y][Ca.x+1]=1;
}
...
int sprawdz()
{
     ...
     //sprawdzam czy czarny krol moze sie ruszyc na bezpieczne miejsce
     bool moze=0;
     if(C.y>0)
     {
              if(C.x>0)
...
{
       atakowa();
       if(ac[z.y][z.x]==0&&ak[z.y][z.x]==0) //zmienione !!!
       {
        tmp=t[z.y][z.x];
        t[z.y][z.x]='B';
        t[k.y][k.x]='.';
